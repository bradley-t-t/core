...
/**
 * Adds a player to the trusted set of a claim.
...
@CommandInfo(
        ...
        playerOnly = true,
        minArgs = 1,
        description = "Add a player to your claim",
        usage = "/trust <player> [claimname]",
        icon = Material.PLAYER_HEAD
)
public final class TrustCommand extends BaseCommand {

    public TrustCommand(CorePlugin plugin) {
        super(plugin);
    }
    ...
    protected void execute(CommandContext context) {
        ...
        if (claim.trustedPlayerIds().contains(target.getUniqueId())) {
            Lang.send(player, "claim.already-trusted", "player", target.getName());
            return;
        }

        claimService.addTrusted(claim.claimId(), target.getUniqueId());
        Lang.send(player, "claim.trusted", "player", target.getName());
